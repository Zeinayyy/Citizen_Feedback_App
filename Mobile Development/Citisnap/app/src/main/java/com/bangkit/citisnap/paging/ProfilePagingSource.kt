...
class ProfilePagingSource(private val userId: String) : PagingSource<DocumentSnapshot, Posts>() {
    override suspend fun load(params: LoadParams<DocumentSnapshot>): LoadResult<DocumentSnapshot, Posts> {
        return try {
            ...
            val collectionRef = firestore.collection("users").document(userId).collection("posts")
            val query = collectionRef.orderBy("timestamp", Query.Direction.DESCENDING)
